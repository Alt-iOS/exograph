defmodule Exograph.AST.Terms do
  @moduledoc """
  Extracts conservative, indexable terms from Elixir AST patterns and source ASTs.
  """
  ...
  def high_signal?("struct:" <> _), do: true
  def high_signal?("struct.field:" <> _), do: true
  ...
  defp visit({:defmodule, _meta, [module_ast, body]} = node, terms, mode) do
    ...
    terms =
      if literal_alias?(module_ast, mode) do
  ...
  defp visit({:%, _meta, [struct_ast, {:%{}, _, fields}]} = node, terms, mode)
       when is_list(fields) do
    terms = ["node:struct" | terms]

    terms =
      if literal_alias?(struct_ast, mode) do
  ...
  defp visit({:__aliases__, _meta, parts} = node, terms, mode) when is_list(parts) do
    if literal_alias?(node, mode),
  ...
  defp pattern_capture_call?(_, _mode), do: false

  defp literal_alias?({:__aliases__, _, parts}, _mode), do: Enum.all?(parts, &is_atom/1)
  defp literal_alias?(_ast, _mode), do: false
